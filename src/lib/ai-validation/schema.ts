...
export const AiLocalGenerationOutputSchema = z.object({
  ...
  narrative: z.object({
    ...
    internal_history: z.string().trim().min(1).max(5_000),
    final_justification: z.string().trim().min(1).max(5_000),
    decision_change_condition: z.string().trim().min(1).max(5_000).nullable().default(null),
  }),
  rationale: z.string().trim().min(1).max(10_000),
  risks: z.array(z.string().trim().min(1).max(2_000)).max(10).default([]),
  invalidation_condition: z.string().trim().min(1).max(5_000),
  limitations: z.array(z.string().trim().min(1).max(2_000)).max(10).default([]),
  sources: z
    .array(
      z.object({
        title: z.string().trim().min(1).max(500),
    ...
    .max(50)
    .default([]),
  searches: z.array(z.string().trim().min(1).max(1_000)).max(50).default([]),
});
