CREATE OR REPLACE FUNCTION public.requeue_highlightly_dead_football_identity_jobs(
  p_scope text,
...
BEGIN
  ...
  WITH candidates AS (
    ...
    WHERE job.shadow_scope = btrim(p_scope)
      ...
      AND job.endpoint_key = 'football.MatchesController_getMatches'
      AND job.status = 'dead'
  ...
  SET
    ...
    cursor_data = COALESCE(job.cursor_data, '{}'::jsonb) || jsonb_build_object(
      '_football_identity_requeued_at', now(),
      '_football_identity_prior_attempts', job.attempts,
      '_football_identity_prior_fanout', job.request_params -> '_fanout'
    ),
    updated_at = now()
  FROM candidates
...
REVOKE ALL ON FUNCTION public.requeue_highlightly_dead_football_identity_jobs(
  text,
  integer
) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.requeue_highlightly_dead_football_identity_jobs(
  text,
  integer
) TO service_role;

COMMENT ON FUNCTION public.requeue_highlightly_dead_football_identity_jobs(
  text,
  integer
) IS
  'Requeues a bounded set of dead football match-list jobs caused by provider participant identity collisions.';
