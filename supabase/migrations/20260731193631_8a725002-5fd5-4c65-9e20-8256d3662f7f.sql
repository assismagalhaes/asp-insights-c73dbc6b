CREATE INDEX IF NOT EXISTS idx_sports_odds_consensus_prematch_latest
  ON public.sports_odds_consensus (match_id, market_definition_id, selection_key, line_key, snapshot_at DESC)
  WHERE NOT is_live;

ANALYZE public.sports_odds_consensus;
ANALYZE public.sports_odds_current;
ANALYZE public.sports_market_definitions;
ANALYZE public.sports_bookmakers;
...
match_base AS MATERIALIZED (
  ...
  ) AS participants ON participants.match_id = match.match_id
...
latest_consensus AS MATERIALIZED (
  SELECT DISTINCT ON (
...
candidates AS (
  ...
  FROM latest_consensus AS consensus
  JOIN match_base AS match ON match.match_id = consensus.match_id
