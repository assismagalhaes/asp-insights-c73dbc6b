  ...
  IF NEW.odd_ajustada > 1 AND TG_OP = 'INSERT' THEN
    ...
    VALUES
      (
        ...
        'VALIDACAO',
        NEW.odd_ajustada,
        NEW.probabilidade_final,
        (NEW.odd_ajustada * NEW.probabilidade_final / 100.0 - 1.0) * 100.0,
        'VALIDACAO_CRITICA'
      );
...
BEGIN
  IF NEW.odd_fechamento > 1 AND TG_OP = 'INSERT' THEN
    SELECT probabilidade_final INTO prediction_probability
    ...
    VALUES
      (
        ...
        (NEW.odd_fechamento * prediction_probability / 100.0 - 1.0) * 100.0,
        'RESULTADO'
      );
...
WHERE p.odd_ofertada > 1
  AND NOT EXISTS (
    SELECT 1 FROM public.prognostico_odds_historico h
    WHERE h.prognostico_id = p.id AND h.tipo = 'REFERENCIA_PACKBALL'
  );
...
SELECT
  ...
  (p.odd_ajustada * p.probabilidade_final / 100.0 - 1.0) * 100.0,
  'BACKFILL',
  p.updated_at
FROM public.prognosticos p
...
SELECT
  ...
  r.odd_fechamento,
  p.probabilidade_final,
  (r.odd_fechamento * p.probabilidade_final / 100.0 - 1.0) * 100.0,
  'BACKFILL',
  r.created_at
...
WHERE r.odd_fechamento > 1
  AND NOT EXISTS (
    SELECT 1 FROM public.prognostico_odds_historico h
    WHERE h.prognostico_id = r.prognostico_id AND h.tipo = 'FECHAMENTO'
  );
...
SELECT
  ...
  CASE
    WHEN validation_price.odd > 1 AND closing_price.odd > 1
