...
BEGIN
  INSERT INTO public.hl_ingestion_jobs (
    endpoint_key, sport, resource, dedupe_key, request_params, cursor_data,
    priority, scheduled_at, max_attempts, reprocess_raw_object_id
  )
  VALUES (
    p_endpoint_key, p_sport, p_resource, p_dedupe_key,
    COALESCE(p_request_params, '{}'::jsonb), COALESCE(p_cursor_data, '{}'::jsonb),
    p_priority, p_scheduled_at, p_max_attempts, p_reprocess_raw_object_id
  )
...
REVOKE ALL ON FUNCTION public.enqueue_highlightly_ingestion_job(text, text, text, text, jsonb, jsonb, smallint, timestamptz, smallint, uuid) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.claim_highlightly_ingestion_job(text, integer) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.finish_highlightly_ingestion_job(uuid, text, text, text, integer) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.enqueue_highlightly_ingestion_job(text, text, text, text, jsonb, jsonb, smallint, timestamptz, smallint, uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.claim_highlightly_ingestion_job(text, integer) TO service_role;
GRANT EXECUTE ON FUNCTION public.finish_highlightly_ingestion_job(uuid, text, text, text, integer) TO service_role;
