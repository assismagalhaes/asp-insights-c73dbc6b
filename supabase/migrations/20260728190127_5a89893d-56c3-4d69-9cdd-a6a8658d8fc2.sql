...
BEGIN
  ...
  WITH coverage AS (
    ...
    JOIN public.sports_competitions AS competition ON competition.id = daily.competition_id
    LEFT JOIN public.sports_countries AS country ON country.id = daily.country_id
    WHERE daily.observed_on >= current_date - (p_days - 1)
    GROUP BY sport.code, country.name, competition.name, daily.competition_id
  ),
  scored AS (
    SELECT
      coverage.*,
      round(100.0 * coverage.matches_available / NULLIF(coverage.matches_due, 0), 2) AS raw_availability_pct,
      round(100.0 * coverage.matches_available / NULLIF(coverage.matches_due - coverage.matches_provider_empty, 0), 2) AS eligible_availability_pct,
      round(100.0 * coverage.matches_provider_empty / NULLIF(coverage.matches_due, 0), 2) AS provider_empty_pct
    FROM coverage
  ),
  recommendations AS (
    SELECT
      scored.*,
      CASE
        WHEN scored.observed_days < p_days OR scored.matches_due < p_min_matches THEN 'insufficient_sample'
  ...
  SELECT jsonb_build_object(
    ...
    'leagues', COALESCE(
      jsonb_agg(
        ...
        ORDER BY recommendations.matches_due DESC, recommendations.sport, recommendations.country_name, recommendations.competition_name
      ),
      '[]'::jsonb
    )
  )
...
REVOKE ALL ON FUNCTION public.get_highlightly_odds_league_coverage_report(integer, integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_highlightly_odds_league_coverage_report(integer, integer) TO authenticated, service_role;

COMMENT ON FUNCTION public.get_highlightly_odds_league_coverage_report(integer, integer) IS 'Admin league coverage report that requires the full observation window and minimum sample before recommendations.';
