...
-- This migration does not enable policies, providers, timers or jobs.
...
BEGIN
  IF p_limit < 1 OR p_limit > 300 THEN
    RAISE EXCEPTION 'limit must be between 1 and 300'
      USING ERRCODE = '22023';
  END IF;
  IF EXISTS (
    SELECT 1
  ...
  WITH candidates AS (
    ...
    WHERE job.shadow_scope LIKE 'phase8e-lifecycle-%'
      ...
      AND job.endpoint_key =
        'football.FootballPlayerBoxScoreController_getPlayerBoxScores'
      AND job.status = 'dead'
      AND job.last_error LIKE
        'Missing path parameters for football.FootballPlayerBoxScoreController_getPlayerBoxScores:%'
      AND COALESCE(
        job.request_params ->> 'matchId',
        job.request_params ->> 'id'
      ) IS NOT NULL
  ...
  repaired AS (
    ...
    SET
      ...
      last_error = NULL,
      request_params = (
        COALESCE(job.request_params, '{}'::jsonb) - 'id'
      ) || jsonb_build_object(
        'matchId',
        COALESCE(
          job.request_params ->> 'matchId',
      ...
      cursor_data = COALESCE(job.cursor_data, '{}'::jsonb)
        || jsonb_build_object(
          '_phase8e3_match_id_repaired_at', now(),
          '_phase8e3_prior_attempts', job.attempts
        ),
      updated_at = now()
    FROM candidates
...
REVOKE ALL ON FUNCTION
  public.get_highlightly_match_lifecycle_operational_report_v2(
    timestamptz,
    timestamptz
  ) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION
...
BEGIN
  WITH scope_catalog AS (
    ...
    UNION ALL
    SELECT
      ingestion_job.shadow_scope,
      'lifecycle'::text,
      CASE
        WHEN count(*) FILTER (
          WHERE ingestion_job.status IN ('pending', 'running', 'retry')
        ) > 0 THEN 'running'
        WHEN count(*) FILTER (WHERE ingestion_job.status = 'dead') > 0
      ...
      CASE
        WHEN count(*) FILTER (
          WHERE ingestion_job.status IN ('pending', 'running', 'retry')
        ) = 0 THEN max(ingestion_job.updated_at)
      END,
  ...
  SELECT catalog.scope, catalog.kind
  INTO selected_scope, selected_kind
  FROM scope_catalog AS catalog
  WHERE NULLIF(btrim(p_scope), '') IS NULL
     OR catalog.scope = btrim(p_scope)
  ORDER BY
    CASE WHEN catalog.scope = btrim(p_scope) THEN 0 ELSE 1 END,
    catalog.updated_at DESC,
    catalog.started_at DESC
  LIMIT 1;
  ...
  SELECT COALESCE(
    jsonb_agg(to_jsonb(catalog) ORDER BY catalog.updated_at DESC),
    '[]'::jsonb
  )
  INTO scope_rows
  FROM (
  ...
  WITH run_requests AS (
    SELECT
      ingestion_run.id AS run_id,
      ingestion_job.shadow_scope,
      ingestion_run.http_status,
    ...
    WHERE (ingestion_run.started_at AT TIME ZONE 'UTC')::date
      = (now() AT TIME ZONE 'UTC')::date
    GROUP BY ingestion_run.id, ingestion_job.shadow_scope, ingestion_run.http_status
  )
  SELECT COALESCE(
    jsonb_agg(to_jsonb(usage_rollup) ORDER BY usage_rollup.collector),
    '[]'::jsonb
  )
  ...
  FROM (
    SELECT
      CASE
        WHEN run_request.shadow_scope LIKE 'phase8e-lifecycle-%'
          THEN 'lifecycle'
        WHEN run_request.shadow_scope LIKE 'phase8d-odds-%'
          THEN 'odds'
        WHEN run_request.shadow_scope LIKE 'future-%'
          THEN 'future_window'
        WHEN run_request.shadow_scope LIKE 'phase7-%'
          THEN 'historical'
        ELSE 'other'
      END AS collector,
      COALESCE(sum(
        CASE
          WHEN run_request.measured_requests > 0
            THEN run_request.measured_requests
          WHEN run_request.http_status IS NOT NULL
            THEN 1
          ELSE 0
        END
      ), 0)::bigint AS requests_used
    FROM run_requests AS run_request
    GROUP BY collector
  ) AS usage_rollup;

  IF selected_kind = 'lifecycle' THEN
    SELECT
      jsonb_build_object(
        'total', count(*),
      ...
      CASE
        ...
        WHEN count(*) FILTER (WHERE status = 'dead') > 0
          THEN 'completed_with_exceptions'
        ELSE 'passed'
      END,
      jsonb_build_object(
    ...
    SELECT COALESCE(
      jsonb_agg(to_jsonb(row_data) ORDER BY row_data.sport),
      '[]'::jsonb
    )
    ...
    SELECT COALESCE(
      jsonb_agg(
        to_jsonb(row_data)
        ORDER BY row_data.sport, row_data.endpoint_key
      ),
      '[]'::jsonb
    )
    ...
    SELECT COALESCE(
      jsonb_agg(to_jsonb(row_data) ORDER BY row_data.updated_at DESC),
      '[]'::jsonb
    )
...
REVOKE ALL ON FUNCTION public.get_highlightly_collection_monitor_v2(text)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_highlightly_collection_monitor_v2(text)
  TO authenticated, service_role;
