-- Applying supabase/migrations/20260715052000_create_highlightly_football_read_models.sql
CREATE OR REPLACE VIEW public.sports_football_match_summary_v
...
JOIN public.sports AS sport ON sport.id = match_row.sport_id AND sport.code = 'football'
...
REVOKE ALL ON public.sports_football_match_summary_v FROM PUBLIC, anon, authenticated;
GRANT SELECT ON public.sports_football_match_summary_v TO authenticated;

CREATE OR REPLACE FUNCTION public.get_football_daily_matches(
  p_from timestamptz,
  p_to timestamptz,
...
RETURNS SETOF public.sports_football_match_summary_v
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  ...
  RETURN QUERY
  SELECT summary.*
  FROM public.sports_football_match_summary_v AS summary
...
CREATE OR REPLACE FUNCTION public.get_football_match_detail(p_match_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
AS $function$
DECLARE
  summary public.sports_football_match_summary_v;
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin'::public.app_role) THEN
    RAISE EXCEPTION 'admin role required' USING ERRCODE = '42501';
  END IF;
  ...
  IF summary.match_id IS NULL THEN
    RAISE EXCEPTION 'football match not found: %', p_match_id USING ERRCODE = 'P0002';
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'teamStatistics', COALESCE((
      ...
      ) ORDER BY fact.team_id, metric.group_name, metric.display_name, fact.period_key)
    ...
    'oddsConsensus', COALESCE((
      SELECT jsonb_agg(to_jsonb(latest_consensus) ORDER BY latest_consensus.selection_key)
      FROM (
        SELECT DISTINCT ON (
    ...
    'lineups', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'players', COALESCE((
          SELECT jsonb_agg(jsonb_build_object(
            ...
            'formationRow', member.formation_row,
            'formationOrder', member.formation_order,
            'metadata', member.metadata
          ) ORDER BY member.role, member.formation_row NULLS LAST, member.formation_order NULLS LAST, player.name)
      ...
      ) ORDER BY lineup.team_id, lineup.published_at DESC NULLS LAST, lineup.created_at DESC)
      FROM public.sports_lineups AS lineup
      WHERE lineup.match_id = p_match_id
        AND lineup.id IN (
          SELECT DISTINCT ON (latest_lineup.team_id) latest_lineup.id
          FROM public.sports_lineups AS latest_lineup
          WHERE latest_lineup.match_id = p_match_id
          ORDER BY latest_lineup.team_id, latest_lineup.published_at DESC NULLS LAST, latest_lineup.created_at DESC
        )
    ), '[]'::jsonb),
    'events', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
      ...
      ) ORDER BY event.elapsed_seconds NULLS LAST, event.sequence_key)
    ...
    'playerBoxScores', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'booleanValue', fact.boolean_value,
        'jsonValue', fact.json_value
      ) ORDER BY fact.team_id, player.name, metric.display_name, fact.period_key)
...
REVOKE ALL ON FUNCTION public.get_football_daily_matches(
  timestamptz, timestamptz, timestamptz, uuid, integer
) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_football_daily_matches(
  timestamptz, timestamptz, timestamptz, uuid, integer
) TO authenticated;

REVOKE ALL ON FUNCTION public.get_football_match_detail(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_football_match_detail(uuid) TO authenticated;
