-- Public prediction contract: mercado + pick. The legacy linha column remains
-- nullable for schema compatibility, but is no longer populated by the app.
...
RETURNS TABLE (market_out text, pick_out text)
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  IF v_line IS NOT NULL AND v_line ~ '^[+-]?[0-9]+([.][0-9]+)?$' THEN
    IF left(v_line, 1) IN ('+', '-') THEN
      v_signed_line := v_line;
    ELSIF v_line::numeric >= 0 THEN
      v_signed_line := '+' || v_line;
    ELSE
      v_signed_line := v_line;
    END IF;
  ...
  IF v_pick_lower ~ '(^| )(casa|mandante|home)( |$)'
     OR (coalesce(p_home, '') <> '' AND v_pick_lower LIKE '%' || lower(p_home) || '%') THEN
    v_side := 'home';
  ELSIF v_pick_lower ~ '(^| )(visitante|fora|away)( |$)'
     OR (coalesce(p_away, '') <> '' AND v_pick_lower LIKE '%' || lower(p_away) || '%') THEN
    v_side := 'away';
  ELSIF v_pick_lower LIKE '%empate%' OR v_pick_lower = 'x' THEN
    v_side := 'draw';
  END IF;

  IF v_pick_lower LIKE '%under%' OR v_market LIKE '%under%' THEN
    v_direction := 'under';
  ELSIF v_pick_lower LIKE '%over%' OR v_market LIKE '%over%' THEN
    v_direction := 'over';
  END IF;

  IF v_pick_lower ~ '(não|nao| no)( |$)' OR v_market ~ '(não|nao| no)( |$)' THEN
    v_btts := 'no';
  ELSIF v_pick_lower LIKE '%btts%' OR v_pick_lower LIKE '%sim%' OR v_market LIKE '%btts%' OR v_market LIKE '%ambas%' THEN
    v_btts := 'yes';
  END IF;
  ...
  ELSIF v_model LIKE '%matchmatrix%' THEN
    IF v_market LIKE '%dupla%' OR v_market LIKE '%double chance%' THEN
      market_out := 'Dupla Chance';
      pick_out := upper(replace(v_pick, ' ', ''));
...
WITH standardized AS (
  ...
  CROSS JOIN LATERAL public._standardize_prediction_market_pick(
    p.origem_modelo, p.esporte, p.mercado, p.pick, p.linha,
...
SET mercado = p.mercado,
    pick = p.pick,
    linha = NULL,
    updated_at = now()
FROM public.prognosticos p
WHERE a.prognostico_id = p.id;

UPDATE public.feedback_ia_resultados f
SET mercado = p.mercado,
    pick = p.pick,
    linha = NULL,
    updated_at = now()
FROM public.prognosticos p
WHERE f.prognostico_id = p.id;
