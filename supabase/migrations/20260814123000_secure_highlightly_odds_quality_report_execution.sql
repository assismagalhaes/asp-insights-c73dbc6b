-- Execute the admin-only report with owner privileges after an explicit JWT/service gate.
-- This avoids re-evaluating RLS across the report's internal joins.

-- Make the existing partial odds-job lookup index usable by the quality report.
-- No data, provider state, schedules, or collection policies are changed.
  ...
  base AS (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE ingestion_job.sport = sport.code
        ...
        AND ingestion_job.endpoint_key IN (
