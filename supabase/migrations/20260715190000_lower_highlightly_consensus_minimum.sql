-- Highlightly global odds policy: calculate consensus with 2-7 preferred bookmakers.

CREATE OR REPLACE FUNCTION public.refresh_sports_odds_consensus(
  p_match_id uuid,
  p_snapshot_at timestamptz DEFAULT now(),
  p_min_bookmakers integer DEFAULT 2,
...
BEGIN
  ...
  END IF;
  IF p_max_bookmakers < 2 OR p_max_bookmakers > 7 THEN
    RAISE EXCEPTION 'p_max_bookmakers must be between 2 and 7';
  END IF;
  IF p_min_bookmakers < 2 OR p_min_bookmakers > p_max_bookmakers THEN
    RAISE EXCEPTION 'p_min_bookmakers must be between 2 and p_max_bookmakers';
  END IF;
