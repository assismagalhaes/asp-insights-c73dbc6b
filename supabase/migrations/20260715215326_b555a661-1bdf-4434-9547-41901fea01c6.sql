  ...
  RETURN jsonb_build_object(
    ...
    'odds', COALESCE((
      ...
      WHERE quote.match_id = p_match_id
        AND market.canonical_family IN ('moneyline', 'total', 'spread')
    ), '[]'::jsonb),
    ...
    'highlights', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'durationSeconds', highlight.duration_seconds,
        'publishedAt', highlight.published_at
      ) ORDER BY highlight.published_at DESC NULLS LAST, highlight.id)
...
REVOKE ALL ON FUNCTION public.get_basketball_daily_matches(timestamptz, timestamptz, timestamptz, uuid, integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_basketball_daily_matches(timestamptz, timestamptz, timestamptz, uuid, integer) TO authenticated;
