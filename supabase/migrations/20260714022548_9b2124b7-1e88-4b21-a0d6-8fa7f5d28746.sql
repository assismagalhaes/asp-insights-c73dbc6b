...
CREATE INDEX IF NOT EXISTS idx_analises_ia_scope ON public.analises_ia(esporte, mercado, liga, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_analises_ia_decisao_sugerida ON public.analises_ia(decisao_sugerida);
CREATE UNIQUE INDEX IF NOT EXISTS idx_analises_ia_validacao_unique ON public.analises_ia(validacao_id) WHERE validacao_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_feedback_ia_resultados_prognostico_id ON public.feedback_ia_resultados(prognostico_id);
CREATE INDEX IF NOT EXISTS idx_feedback_ia_resultados_scope ON public.feedback_ia_resultados(esporte, mercado, liga, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_feedback_ia_resultados_decisao_ia ON public.feedback_ia_resultados(decisao_ia_sugerida);
CREATE UNIQUE INDEX IF NOT EXISTS idx_feedback_ia_resultados_unique_analysis ON public.feedback_ia_resultados(prognostico_id, analise_ia_id);

DROP TRIGGER IF EXISTS trg_analises_ia_touch_updated_at ON public.analises_ia;
CREATE TRIGGER trg_analises_ia_touch_updated_at BEFORE UPDATE ON public.analises_ia FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();

DROP TRIGGER IF EXISTS trg_feedback_ia_touch_updated_at ON public.feedback_ia_resultados;
CREATE TRIGGER trg_feedback_ia_touch_updated_at BEFORE UPDATE ON public.feedback_ia_resultados FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();

INSERT INTO public.analises_ia (
  prognostico_id, validacao_id, modo_ia, esporte, liga, mercado, pick, linha, jogo, data_evento, hora_evento,
  odd_usada, probabilidade_final, edge_usado, contexto_analisado, parecer_ia, decisao_sugerida, stake_sugerida,
  riscos_identificados, fontes_consultadas, buscas_realizadas, prompt_versao, created_at, updated_at
)
SELECT
  p.id, v.id, COALESCE(NULLIF(v.modo_ia, ''), 'desconhecido'),
  p.esporte, p.liga, p.mercado, p.pick, NULL, p.jogo, p.data, p.hora,
  COALESCE(p.odd_ajustada, p.odd_ofertada), p.probabilidade_final, COALESCE(p.edge_ajustado, p.edge),
  v.contexto_adicional, v.parecer_ia,
  CASE
    ...
    ELSE NULL
  END,
  v.stake_ia_sugerida, v.parecer_ia, v.fontes_consultadas, v.buscas_realizadas, v.prompt_versao,
...
WHERE v.decisao_ia_sugerida IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM public.analises_ia a WHERE a.validacao_id = v.id);
...
BEGIN
  normalized_result := CASE
    ...
    ELSE NULL
  END;
  IF normalized_result IS NULL THEN RETURN NEW; END IF;

  SELECT * INTO p FROM public.prognosticos WHERE id = NEW.prognostico_id;
  IF NOT FOUND THEN RETURN NEW; END IF;

  SELECT * INTO v FROM public.validacoes WHERE prognostico_id = NEW.prognostico_id ORDER BY created_at DESC LIMIT 1;
  SELECT * INTO a FROM public.analises_ia WHERE prognostico_id = NEW.prognostico_id AND decisao_sugerida IS NOT NULL ORDER BY created_at DESC LIMIT 1;
  ...
  IF a.id IS NULL AND v.id IS NOT NULL AND v.decisao_ia_sugerida IS NOT NULL THEN
    INSERT INTO public.analises_ia (
      ...
      p.id, v.id, COALESCE(NULLIF(v.modo_ia, ''), 'desconhecido'), p.esporte, p.liga, p.mercado, p.pick, NULL, p.jogo, p.data, p.hora,
      COALESCE(p.odd_ajustada, p.odd_ofertada), p.probabilidade_final, COALESCE(p.edge_ajustado, p.edge),
      v.contexto_adicional, v.parecer_ia, v.decisao_ia_sugerida, v.stake_ia_sugerida, v.parecer_ia,
    ...
    ON CONFLICT (validacao_id) WHERE validacao_id IS NOT NULL DO UPDATE
      SET parecer_ia = EXCLUDED.parecer_ia, decisao_sugerida = EXCLUDED.decisao_sugerida,
          stake_sugerida = EXCLUDED.stake_sugerida, updated_at = now()
    RETURNING * INTO a;
  END IF;
  ...
  INSERT INTO public.feedback_ia_resultados (
    ...
    buscas_realizadas, acertou_ia, acertou_humano, divergencia_ia_humano, created_at, updated_at
  ) VALUES (
    p.id, a.id, a.modo_ia, p.esporte, p.liga, p.mercado, p.pick, NULL, p.jogo,
    ai_decision, a.stake_sugerida, human_decision, COALESCE(v.stake_confirmada, p.stake),
    ...
    CASE WHEN counts_bankroll THEN NEW.lucro_prejuizo ELSE 0 END,
    COALESCE(p.odd_ajustada, p.odd_ofertada), p.probabilidade_final, COALESCE(p.edge_ajustado, p.edge),
    a.tags_risco, a.fontes_consultadas, a.buscas_realizadas,
    CASE WHEN ai_decision = 'CONFIRMAR' THEN normalized_result = 'GREEN' ELSE normalized_result = 'RED' END,
    CASE WHEN human_decision = 'CONFIRMAR' THEN normalized_result = 'GREEN'
         WHEN human_decision = 'PULAR' THEN normalized_result = 'RED' ELSE NULL END,
...
SELECT
  p.id, a.id, a.modo_ia, p.esporte, p.liga, p.mercado, p.pick, NULL, p.jogo,
  d.ai_decision, a.stake_sugerida, d.human_decision, COALESCE(v.stake_confirmada, p.stake),
  d.normalized_result, d.normalized_result,
  ...
  r.lucro_prejuizo, r.lucro_prejuizo,
  CASE WHEN d.human_decision = 'CONFIRMAR' THEN r.lucro_prejuizo ELSE 0 END,
  COALESCE(p.odd_ajustada, p.odd_ofertada), p.probabilidade_final, COALESCE(p.edge_ajustado, p.edge),
  a.tags_risco, a.fontes_consultadas, a.buscas_realizadas,
  ...
  d.ai_decision <> d.human_decision, r.created_at, now()
FROM public.prognosticos p
JOIN LATERAL (SELECT * FROM public.resultados rr WHERE rr.prognostico_id = p.id ORDER BY rr.created_at DESC LIMIT 1) r ON true
JOIN LATERAL (SELECT * FROM public.validacoes vv WHERE vv.prognostico_id = p.id AND vv.decisao_ia_sugerida IS NOT NULL ORDER BY vv.created_at DESC LIMIT 1) v ON true
JOIN LATERAL (SELECT * FROM public.analises_ia aa WHERE aa.prognostico_id = p.id AND aa.decisao_sugerida IS NOT NULL ORDER BY aa.created_at DESC LIMIT 1) a ON true
CROSS JOIN LATERAL (
  SELECT
    CASE WHEN upper(COALESCE(r.resultado, '')) IN ('GREEN', 'WIN', 'WINS') THEN 'GREEN'
         WHEN upper(COALESCE(r.resultado, '')) IN ('RED', 'LOSS', 'LOSSES') THEN 'RED' ELSE NULL END AS normalized_result,
    CASE WHEN upper(COALESCE(a.decisao_sugerida, v.decisao_ia_sugerida, '')) LIKE ANY (ARRAY['%PULAR%', '%PASS%', '%AGUARDAR%']) THEN 'PULAR'
         WHEN upper(COALESCE(a.decisao_sugerida, v.decisao_ia_sugerida, '')) LIKE '%CONFIRMA%' THEN 'CONFIRMAR' ELSE NULL END AS ai_decision,
    CASE WHEN upper(COALESCE(v.decisao, p.status_validacao, '')) LIKE '%CONFIRMA%' THEN 'CONFIRMAR'
         WHEN upper(COALESCE(v.decisao, p.status_validacao, '')) LIKE ANY (ARRAY['%PULAR%', '%PASS%', '%AGUARDAR%']) THEN 'PULAR' ELSE NULL END AS human_decision
) d
WHERE d.normalized_result IS NOT NULL AND d.ai_decision IS NOT NULL AND d.human_decision IS NOT NULL
ON CONFLICT (prognostico_id, analise_ia_id) DO UPDATE SET
  resultado_real = EXCLUDED.resultado_real,
  resultado_teorico = EXCLUDED.resultado_teorico,
  resultado_financeiro = EXCLUDED.resultado_financeiro,
