...
def _normalize_matches(payload: Any, ctx: NormalizationContext) -> NormalizedBatch:
    ...
    for match in items(payload):
        ...
        home_external = (
            _external(home.get("id"), f"name:{slug(home.get('name'))}")
            if home
            else None
        )
        away_external = (
            _external(away.get("id"), f"name:{slug(away.get('name'))}")
            if away
            else None
        )
        if home_external and away_external and home_external == away_external:
            ...
            batch.issue(
                ...
                "Football match maps home and away to the same provider team and was not persisted.",
                context={
                    "matchId": external_id,
