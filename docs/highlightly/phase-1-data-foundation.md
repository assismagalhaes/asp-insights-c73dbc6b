...
Data: 15/07/2026
Estado: aplicada e validada no banco ativo
...
1. `20260715025742_b36aa56c-6997-44e5-b621-be2d9f17f35e.sql`;
2. `20260715025933_9a7e5a0e-3b75-40ef-af71-2e3fac97b16a.sql`;
3. `20260715030012_eeaf5ecd-3809-4de1-91e1-e39f673e2a86.sql`.
...
O bucket não é criado por escrita direta em `storage.buckets`, pois o ambiente Lovable Cloud rejeita essa operação em migrations. O provisionamento idempotente usa a Storage API oficial:

```powershell
python -m scripts.ensure_highlightly_storage_bucket
```

O comando cria ou reconcilia `highlightly-raw` como privado, com limite de 25 MiB e MIME types JSON/gzip. Requer `SUPABASE_URL` e `SUPABASE_SERVICE_ROLE_KEY` somente no ambiente do servidor/deploy.
...
Executar `supabase/tests/highlightly_phase1_smoke.sql` após aplicar as três migrations e provisionar o bucket. O script roda em transação e termina com `ROLLBACK`; ele verifica:
...
```text
PASS migration 20260715025742_b36aa56c-6997-44e5-b621-be2d9f17f35e.sql
PASS migration 20260715025933_9a7e5a0e-3b75-40ef-af71-2e3fac97b16a.sql
PASS migration 20260715030012_eeaf5ecd-3809-4de1-91e1-e39f673e2a86.sql
PASS transactional smoke
PASS post-smoke state {"providers":1,"sports":3,"bookmakers":11,"jobs":0,"bucket_public":false}
```

Além disso, 31 testes Python/contratuais passaram e o typecheck TypeScript terminou sem erros.

## Estado da aplicação no banco

As três migrations foram aplicadas pelo Lovable em 15/07/2026 e passaram pelas verificações diretas no banco:

- 13 tabelas canônicas e 5 tabelas de ingestão existentes;
- três RPCs de fila existentes;
- provider Highlightly em `contract_version=6.13.2` e `enabled=false`;
- 3 esportes e 11 bookmakers semeados;
- bucket `highlightly-raw` existente e `public=false`;
- `anon` sem `SELECT` nas tabelas internas;
- `authenticated` sujeito a RLS admin-only;
- `anon`/`authenticated` sem `EXECUTE` nas RPCs;
- `service_role` com `EXECUTE` nas RPCs.

O sandbox SQL usado na aplicação não tinha privilégio para executar `SET ROLE authenticated`. As asserções correspondentes do smoke foram verificadas diretamente por grants, policies, roles e estado das tabelas. Nenhuma feature flag foi ativada.
