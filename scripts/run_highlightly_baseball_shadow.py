"""Execute one bounded MLB shadow and always restore the provider kill switch."""
...
MATCH_ENDPOINT = "baseball.BaseballMatchController_getMatchById"
DISCOVERY_ENDPOINT = "baseball.BaseballMatchController_getMatches"
...
def _discover_match(
    ...
    for offset in range(3):
        ...
        response = client.get("/baseball/matches", params)
        repository.store_raw_payload(
            ...
            request_metadata={"path": "/baseball/matches", "params": params, "purpose": "bounded_shadow_discovery"},
        ...
        preferred = next(
            (
                ...
                in {"not started", "scheduled", "in progress"}
            ),
            matches[0],
        )
        return str(preferred["id"]), dict(preferred)
    raise RuntimeError("No MLB match was returned for the requested three-day discovery window")
...
def _validate_shadow(
    ...
    *,
    ...
    counts = {
        ...
        "inning_scores": _count(repository, "sports_match_period_scores", {"match_id": match_id}),
...
def main() -> int:
    ...
    parser.add_argument("--match-id", help="Highlightly MLB match id; omit to discover an upcoming MLB match")
    ...
    shadow_scope = f"mlb-shadow-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}-{uuid4().hex[:8]}"
    ...
    provider_after = repository.ingestion_context("baseball")["provider"]
