"""Replay dead football match-list jobs caused by duplicate participant identity."""
...
REQUEUE_RPC = "requeue_highlightly_dead_football_identity_jobs"
ERROR_MARKERS = (
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--confirm-football-identity-replay", action="store_true")
    ...
    if provider.get("enabled"):
        raise RuntimeError(
            "Highlightly provider must be disabled before the football identity replay"
        )
    if _active_jobs(repository):
        raise RuntimeError(
            "Active ingestion queue must be empty before the football identity replay"
        )
    ...
    report: dict[str, Any] = {
        "scope": args.scope,
        "mode": (
            "execute" if args.confirm_football_identity_replay else "dry-run"
        ),
    ...
    if not args.confirm_football_identity_replay or not candidates:
    ...
    try:
        worker = HighlightlyWorker(
            client,
            repository,
            worker_id=f"{args.scope}:football-identity-replay",
            enabled=True,
    ...
    report.update(
        {
            ...
            "recommended_action": (
                ...
                if completed
                else "stop_and_review_football_identity_replay"
            ),
        }
    )
