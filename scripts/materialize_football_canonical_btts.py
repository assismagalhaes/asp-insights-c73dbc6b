"""Materialize deterministic canonical BTTS shadow predictions from frozen inputs."""
...
def materialize(canonical_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    source_manifest = json.loads((canonical_dir / "manifest.json").read_text(encoding="utf-8"))
    prepared = []
    for item in source_manifest["artifacts"]:
        source = json.loads((canonical_dir / item["path"]).read_text(encoding="utf-8"))
        ...
        inference = infer_canonical_btts(source["snapshot"])
        payload = {
            "contract": "football_canonical_btts_shadow_artifact@1.0.0",
    ...
    manifest = {
        "contract": "football_canonical_btts_shadow_materialization@1.0.0",
...
def main() -> int:
    ...
    keys = ("row_count", "prediction_count", "candidate_count", "missing_quote_count", "provider_calls", "published", "manifest_fingerprint")
