"""Execute one bounded WNBA shadow and always restore the provider kill switch."""
...
MATCH_ENDPOINT = "basketball.MatchesController_getMatchById"
DISCOVERY_ENDPOINT = "basketball.MatchesController_getMatches"
WNBA_LEAGUE_ID = 11847
...
def _discover_match(
    ...
    for offset in range(3):
        ...
        response = client.get("/basketball/matches", params)
        repository.store_raw_payload(
            ...
            request_metadata={"path": "/basketball/matches", "params": params, "purpose": "bounded_shadow_discovery"},
        ...
        finished = next(
            (
                ...
                in {"finished", "final", "completed"}
            ),
            matches[0],
        )
        return str(finished["id"]), dict(finished)
    raise RuntimeError("No WNBA match was returned for the requested three-day discovery window")
...
def _assert_empty_queue(repository: HighlightlyRepository) -> None:
    ...
    for status in ("pending", "retry", "running"):
        rows = repository.select_rows(
            "hl_ingestion_jobs", columns="id,status,endpoint_key", filters={"status": status}, limit=1
        )
        if rows:
...
def _validate_shadow(
    ...
    *,
    ...
    mappings = repository.select_rows(
        ...
        filters={
            "provider_id": provider_id, "sport_id": sport_id,
            "entity_type": "match", "external_id": external_match_id,
        },
        limit=1,
    )
    if not mappings:
    ...
    counts = {
        ...
        "quarter_scores": _count(repository, "sports_match_period_scores", {"match_id": match_id}),
        ...
        "odds_consensus": _count(repository, "sports_odds_consensus", {"match_id": match_id}),
        "highlights": _count(repository, "sports_highlights", {"match_id": match_id}),
    }
    issues = [
        row
        for run_id in run_ids
        for row in repository.select_rows(
            "hl_data_quality_issues", columns="id,issue_code,severity", filters={"run_id": run_id}
        )
    ]
    return {
        "match_mapped": True,
        ...
        "standings_guard_triggered": any(row.get("issue_code") == "BASKETBALL_STANDINGS_CORRUPTED" for row in issues),
    }


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--match-id", help="Highlightly WNBA match id; omit to discover a WNBA match")
    parser.add_argument("--date", type=date.fromisoformat, help="Start date for discovery (YYYY-MM-DD)")
    ...
    if not match_id:
        match_id, discovery = _discover_match(
            client, repository, str(provider["id"]), str(sport["id"]), args.date
        )

    shadow_scope = f"wnba-shadow-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}-{uuid4().hex[:8]}"
    ...
    try:
        ...
        worker = HighlightlyWorker(client, repository, worker_id=shadow_scope, enabled=True)
    ...
    provider_after = repository.ingestion_context("basketball")["provider"]
    validation = _validate_shadow(
        repository,
        provider_id=str(provider["id"]), sport_id=str(sport["id"]),
        external_match_id=str(match_id), run_ids=run_ids,
    )
    report = {
        "scope": shadow_scope,
        ...
        "discovery": {
            ...
            "league": (discovery.get("league") or {}).get("name"),
