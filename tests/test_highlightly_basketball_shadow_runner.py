...
from scripts import run_highlightly_basketball_shadow as shadow


class HighlightlyBasketballShadowRunnerTests(unittest.TestCase):
    @staticmethod
    def _context():
        return {
    ...
    def test_bounded_wnba_shadow_restores_provider(
        self, repository_factory, client_factory, worker_factory,
        assert_empty_queue, validate_shadow,
    ):
        repository = Mock()
        ...
        validate_shadow.return_value = {
            "match_mapped": True, "counts": {}, "quality_issues": 0,
            "standings_guard_triggered": False,
        }

        argv = ["run_highlightly_basketball_shadow", "--match-id", "99", "--confirm-bounded-shadow"]
        ...
        self.assertEqual(job["sport"], "basketball")
        self.assertEqual(job["endpoint_key"], shadow.MATCH_ENDPOINT)
    ...
    @patch.object(shadow, "HighlightlyClient")
    @patch.object(shadow, "HighlightlyRepository")
    def test_unexpected_wnba_worker_exception_still_restores_provider(
        self, repository_factory, client_factory, worker_factory, assert_empty_queue,
    ):
        repository = Mock()
        repository_factory.from_environment.return_value = repository
